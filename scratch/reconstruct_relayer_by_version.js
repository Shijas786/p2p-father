...
function reconstruct(targetTotalLines) {
    ...
    files.forEach(file => {
        ...
        // Check if this response is viewing relayer.ts and matches target total lines
        if (!content.includes('relayer.ts')) return;
    ...
    const outputPath = `/Users/shijas/p2p father/scratch/recovered_relayer_${targetTotalLines}.ts`;
    fs.writeFileSync(outputPath, finalCode.join('\n'));
    console.log(`Reconstructed relayer.ts (${targetTotalLines} lines) saved to: ${outputPath} (Total lines reconstructed: ${finalCode.length})`);
}
