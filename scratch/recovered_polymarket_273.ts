...
class PolymarketService {
    ...
    async getActiveBtcMarket(): Promise<ActiveMarketInfo> {
        try {
            ...
            const targetEpoch = activeEpoch + 300;
            const slug = `btc-updown-5m-${targetEpoch}`;
    ...
    async getOutcomePrice(tokenId: string, isNo: boolean = false): Promise<{ buyPrice: number; sellPrice: number }> {
        const getDynamicFallback = async () => {
            try {
                ...
                const tickerRes = await axios.get("https://api.exchange.coinbase.com/products/BTC-USD/ticker", {
                    headers: { "User-Agent": "Mozilla/5.0" },
                    timeout: 3000
                });
                const d = res.data?.[0];
