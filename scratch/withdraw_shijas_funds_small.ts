...
async function main() {
    ...
    const transferAmount = 1000000n; // Transfer exactly 1 pUSD (1.00)
    console.log(`Withdrawing 1.00 pUSD gaslessly to main wallet: ${recipientAddress}...`);
    
    try {
        const txHash = await polymarketRelayerService.withdrawGasless(userIndex, recipientAddress, transferAmount);
