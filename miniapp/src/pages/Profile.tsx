...
export function Profile({ user, onUpdate, onSwitchWallet }: Props) {
    ...
    const [waCodeData, setWaCodeData] = useState<{ code: string; wa_link: string } | null>(null);
    const [waLoading, setWaLoading] = useState(false);
    const [prefChannel, setPrefChannel] = useState<'telegram' | 'whatsapp' | 'both'>(user?.preferred_channel || 'telegram');
    const waBoxRef = useRef<HTMLDivElement | null>(null);

    useEffect(() => {
        setPrivacyMode(Boolean(user?.hide_group_handle));
        if (user?.preferred_channel) setPrefChannel(user.preferred_channel);

        // Restore active link code from localStorage if unexpired (<10 min)
        try {
            const cached = localStorage.getItem('p2p_wa_link_code');
            if (cached) {
                const parsed = JSON.parse(cached);
                if (Date.now() - (parsed.timestamp || 0) < 600000) {
                    setWaCodeData(parsed);
                } else {
                    localStorage.removeItem('p2p_wa_link_code');
                }
            }
        } catch (_) {}
    }, [user?.hide_group_handle, user?.preferred_channel]);

    // Click outside handler to dismiss OTP box
    useEffect(() => {
        function handleClickOutside(event: Event) {
            if (waBoxRef.current && !waBoxRef.current.contains(event.target as Node)) {
                closeWaBox();
            }
        }
        if (waCodeData) {
    ...
    async function generateWaLinkCode() {
        haptic('medium');

        // If box is already open, toggle close it
        if (waCodeData) {
            closeWaBox();
            return;
        }
        ...
        setWaLoading(true);
        try {
            const res = await api.whatsapp.getLinkCode();
            const dataToSave = { ...res, timestamp: Date.now() };
            setWaCodeData(dataToSave);
            try {
                localStorage.setItem('p2p_wa_link_code', JSON.stringify(dataToSave));
            } catch (_) {}
        } catch (err: any) {
            console.warn('Failed to get WA link code:', err);
            // Fallback for offline/mock testing
            const mockCode = Math.floor(100000 + Math.random() * 900000).toString();
            const mockData = {
                code: mockCode,
                wa_link: `https://wa.me/?text=%2Flink%20${mockCode}`,
                timestamp: Date.now()
            };
            setWaCodeData(mockData);
            try {
                localStorage.setItem('p2p_wa_link_code', JSON.stringify(mockData));
            } catch (_) {}
        } finally {
            setWaLoading(false);
        }
    ...
    async function updateChannelPref(channel: 'telegram' | 'whatsapp' | 'both') {
        haptic('light');
        setPrefChannel(channel);
        try {
            await api.whatsapp.updatePreference(channel);
            haptic('success');
            onUpdate();
        } catch (err) {
            console.warn('Failed to update channel pref:', err);
        }
    }

    async function handleUnlinkWa() {
        if (!window.confirm('Unlink WhatsApp account from your P2PFather profile?')) return;
        haptic('medium');
        try {
            ...
            console.warn('Failed to unlink WA:', err);
        }
    }
        ...
        <div className="page profile-page animate-in" style={{ '--theme-hue': themeHue } as React.CSSProperties}>
            ...
            <div className="prof-header">
                ...
                <div className="prof-info-col" style={{ flex: 1, position: 'relative', zIndex: 1 }}>
                    ...
                    {/* Binance-style Identity Verification Badge / CTA Button (Public Mode) */}
            ...
            <div className="prof-menu-card">
                ...
                {/* 2.6 WhatsApp Integration Card */}
                <div className="prof-payment-item" style={{ flexDirection: 'column', alignItems: 'stretch', gap: '10px' }}>
                    <div className="prof-payment-top">
                        <div style={{ display: 'flex', alignItems: 'center', flex: 1 }}>
                            <img 
                                src="/icons for trade/profile icons/whatsapp-account.svg?v=1" 
                                alt="" 
                                style={{ width: '28px', height: '28px', marginRight: '16px' }} 
                            />
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                <span className="prof-payment-name">WhatsApp Account</span>
                                <span className="prof-payment-value" style={{ fontSize: '11px', color: user?.whatsapp_phone ? '#00e5a0' : '#848e9c' }}>
                                    {user?.whatsapp_phone 
                                        ? `Linked: +${user.whatsapp_phone}` 
                        ...
                        {user?.whatsapp_phone ? (
                            ...
                            <button className="prof-edit-btn" onClick={generateWaLinkCode} disabled={waLoading}>
                                {waLoading ? 'Generating...' : 'Link WA'}
                            </button>
                        )}
                    </div>

                    {/* Display Link Code OTP Box if generated (Compact Monochrome Glassmorphism) */}
                    {waCodeData && !user?.whatsapp_phone && (
                        <div ref={waBoxRef} style={{ 
                            position: 'relative',
                            background: 'linear-gradient(135deg, rgba(255, 255, 255, 0.04) 0%, rgba(15, 17, 23, 0.9) 100%)', 
                            ...
                            <div style={{ fontSize: '9px', color: '#64748B', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '2px', fontWeight: 600 }}>
                                LINK OTP
                            </div>
                            ...
                            <div style={{ fontSize: '10px', color: '#475569', marginBottom: '8px' }}>
                                Valid 10m • Send to WhatsApp bot
                            </div>
                            ...
                            <button 
                                ...
                                onClick={() => {
                                    haptic('light');
                                    window.open(waCodeData.wa_link, '_blank');
                                }}
                            >
                                📲 Open WhatsApp & Link
                            </button>
                    ...
                    {/* Notification Channel Preference selector if linked */}
                    {user?.whatsapp_phone && (
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: '10px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                            <span style={{ fontSize: '11px', color: '#848e9c' }}>Alert Channel:</span>
                            <div style={{ display: 'flex', gap: '6px' }}>
                                {(['telegram', 'whatsapp', 'both'] as const).map(ch => (
                                    <button
                                        key={ch}
                                        onClick={() => updateChannelPref(ch)}
                                        style={{
                                            padding: '5px 12px',
                                            borderRadius: '8px',
                                            fontSize: '11px',
                                            border: 'none',
                                            cursor: 'pointer',
                                            background: prefChannel === ch ? 'linear-gradient(135deg, #25D366 0%, #00e5a0 100%)' : 'rgba(255,255,255,0.06)',
                                            color: prefChannel === ch ? '#042014' : '#848e9c',
                                            fontWeight: prefChannel === ch ? '700' : '500',
                                            boxShadow: prefChannel === ch ? '0 2px 8px rgba(0,229,160,0.2)' : 'none',
                                            transition: 'all 0.2s ease'
                                        }}
                                    >
                                        {ch === 'telegram' ? 'Telegram' : ch === 'whatsapp' ? 'WhatsApp' : 'Both ✨'}
                                    </button>
                                ))}
                            </div>
                        </div>
