...
// Fallback admin Telegram ID (fallback to configured ADMIN_IDS)
const FALLBACK_ADMIN_ID = 123456789;
...
/**
 * Send an alert directly to the Admin on Telegram when Meta updates WhatsApp.
 */
export async function sendMetaUpdateAlertToAdmin(
    oldRev: string,
    newRev: string,
    extraInfo = ""
): Promise<void> {
    const targetId = env.ADMIN_IDS.length > 0
        ? env.ADMIN_IDS[0]
        : FALLBACK_ADMIN_ID;
...
${extraInfo ? `\n_${extraInfo}_` : ""}`;
    ...
    try {
        ...
        console.log(`[WA-META-MONITOR] Alert sent to Admin (TG: ${targetId})`);
    } catch (err: any) {
        console.error("[WA-META-MONITOR] Failed to send Telegram alert to Admin:", err?.message);
    }
}
...
export async function checkMetaWhatsAppUpdate(): Promise<void> {
    try {
        ...
        if (currentRev !== lastKnownRevision) {
            ...
            lastKnownRevision = currentRev;
            await sendMetaUpdateAlertToAdmin(old, currentRev);
        }
    } catch (err: any) {
...
export function startMetaWhatsAppMonitor(): void {
    console.log("  🔍 Meta WhatsApp Web Update Monitor started (Alerts -> Admin on Telegram)");
