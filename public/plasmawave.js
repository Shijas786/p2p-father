import { Renderer, Camera, Transform, Program, Mesh, Geometry } from 'https://esm.sh/ogl';
...
export default function initPlasmaWave(container, options = {}) {
  const {
    xOffset = 0,
    yOffset = 0,
    rotationDeg = 0,
    ...
    bend1 = 1,
    bend2 = 0.5,
    colors = ['#10b981', '#3b82f6'] // Default colors matching the theme
  } = options;
...
function runInit() {
    const containers = document.querySelectorAll('.plasma-wave-container');
    containers.forEach(container => {
        initPlasmaWave(container, {
            colors: ['#047857', '#0369a1'], // Darker green and blue for a sleek background
            speed1: 0.05,
            speed2: 0.05,
            focalLength: 0.8,
            bend1: 1,
